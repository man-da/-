...
<nav ng-controller="sidebarController" class="sidebar  ng-scope squirrel-nav">
    <ul class="squirrel-nav-list">
        <li ng-class="{true: 'active'}[isAll]" class="nav-now">
...
<div class="squirrel-list-container">
    ...
    <div class="squirrel-list-top-nav">
        <span>首页</span>/<span  class="squirrel-list-item-name"><c:if test="${empty catelog.name}"><a style="color: #ffa323;">最新发布</a></c:if>
         <c:if test="${!empty catelog.name}"><a href="" style="color: #ffa323;">${catelog.name}</a></c:if></span>
    </div>
    <div class="squirrel-list-content">

        <div class="squirrel-list-title">
            <h2><c:if test="${empty catelog.name}"><a href="">最新</a></c:if>
...
<script>
    $(function () {
        var itemName =  $('.squirrel-list-item-name').text().trim();
        var item = $(".squirrel-nav-list li");
        var itemIndex ;
        item.removeClass();
        switch (itemName) {
            case  '最新发布':
        ...
        $(".squirrel-nav-list li").eq(itemIndex).addClass('nav-now');
    })
</script>
</body>
</html>